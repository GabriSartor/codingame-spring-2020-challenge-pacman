...
//		            }
		            ...
		            dp.put(new Couple(source, cur), backtrackPath(cur));
...
//		            	c = new Couple(cur, source);
		...
		private static final int safeDistance = 3;
		private static final int minSpread = 4;
		private static final double minStart = 0.9;
		...
		static class Couple {
			...
			public boolean equals(Object obj) {
				...
				return (this.couple.containsAll(other.couple));
			}
	...
	public static void main(String[] args) {
        ...
        Coordinate source1= new Coordinate(1, 1);
        Coordinate source2= new Coordinate(1, 2);
        myPacMap.put(0, new Pac(0, true, source1, PacType.PAPER, 0, 10));
        //Se c'è un Pac non c'è un pellet
        pelletList.put(myPacMap.get(0).getPosition(), 0);
        
		System.out.println("Grafo creato"+graph.getHeight()+" "+graph.getWidth());
		StringBuffer sb = new StringBuffer();
		BFSSolver bfs = new BFSSolver();
		graph.reset();
		List<Coordinate> path = bfs.solve(graph, source1, source2);
		graph.reset();
		List<Coordinate> path2 = bfs.solve(graph, source2, source1);
		for (Pac p:myPacMap.values()) {       		
			Coordinate destination = findBestRoute(p.getPosition(), p.getSpeedTurnsLeft());
...
//        		//continue
//        	if (p.isBusy() && !p.isColliding() && safe && pathValue(dp.get(new Couple (p.getPosition(), p.getDestination())), p.speedTurnsLeft) > 0.0) {
